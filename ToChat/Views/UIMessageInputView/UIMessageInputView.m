...
/**
 *  标记了键盘输入状态（参见UIMessageInputViewState）
 *  修改输入框状态后需要修改按钮的图片
...
/// 因为emoji键盘和输入法键盘的高度不一致，所以切换时需要微调输入框的位置
...
- (void)setInputState:(UIMessageInputViewState)inputState{
    if (_inputState != inputState) {
        ...
        switch (_inputState) {
            ...
            {
                [self.addButton setImage:[UIImage imageNamed:@"keyboard_add"] forState:UIControlStateNormal];//加号
                [self.emotionButton setImage:[UIImage imageNamed:@"keyboard_emotion"] forState:UIControlStateNormal];//emoji
            ...
            {
                [self.addButton setImage:[UIImage imageNamed:@"keyboard_add"] forState:UIControlStateNormal];//加号
                [self.emotionButton setImage:[UIImage imageNamed:@"keyboard_keyboard"] forState:UIControlStateNormal];//键盘
                
            }
                break;
            case UIMessageInputViewStateAdd:
            {
                [self.addButton setImage:[UIImage imageNamed:@"keyboard_keyboard"] forState:UIControlStateNormal];//键盘
                [self.emotionButton setImage:[UIImage imageNamed:@"keyboard_emotion"] forState:UIControlStateNormal];//emoji
...
/**
 *  根据页面类型设定按钮
 *  toolBtnNum最多为2 因为写布局的时候扩展性不够好
 *  如果需要显示更多的数量的按钮需要自己重新修改
 */
- (void)customUIWithType:(UIMessageInputViewContentType)type{
    ...
    //如果需要显示某个按钮并且某个按钮还没有创建过，则创建这个按钮
